...
// Adapted from Shadertoy to WebGL uniforms
...
void main() {
    ...
    // Use const step sizes for WebGL compatibility
    const float stp8 = 1./8.;
    const float stp32 = 1./32.;

    for(float i=0.; i<1.; i+=stp8) {
       col += StreetLights(i, t);
    }

    for(float i=0.; i<1.; i+=stp8) {
        float n = N(i+floor(t));
        col += HeadLights(i+n*stp8*.7, t);
    }

    for(float i=0.; i<1.; i+=stp32) {
